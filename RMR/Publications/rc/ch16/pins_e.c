...
RtPointer shadowfile = "pinshad.txt";
...
Go()                 
{
    ...
    RI_SHDFILE = RiDeclare("shadowfile", "uniform string");
    RiLightSource("shadowspot", (RtToken)"intensity", &intensity, 
        (RtToken)"coneangle", &coneangle, 
        (RtToken)"conedeltaangle", &conedeltaangle, 
        (RtToken)"from", from,
        (RtToken)"to", to,
        RI_SHDFILE, &shadowfile,
        RI_NULL);
