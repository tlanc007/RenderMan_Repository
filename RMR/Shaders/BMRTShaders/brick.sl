...
#define sqr(x) ((x)*(x))
     ...
     * a bit smoother.
     */
    swidth = 1.5 * max (filterwidth(s), MINFILTERWIDTH) / BMWIDTH;
    twidth = 1.5 * max (filterwidth(t), MINFILTERWIDTH) / BMHEIGHT;
    ...
    ss += raggedamp * snoise2 ((s+tbrick*5.15)*raggedfreq,
			       (t+sbrick*23.8)*raggedfreq);
    tt += raggedamp * snoise2 ((s+tbrick*11.4)*raggedfreq,
			       (t+sbrick*7.2)*raggedfreq);
    ss += raggedamp/2 * snoise2 ((s+tbrick*5.15)*raggedfreq*2,
				 (t+sbrick*23.8)*raggedfreq*2);
    tt += raggedamp/2 * snoise2 ((s+tbrick*11.4)*raggedfreq*2,
