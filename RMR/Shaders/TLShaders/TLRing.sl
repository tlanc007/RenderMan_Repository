...
/* Grabbed from one of Larry Gritz's many shaders */
#ifdef BMRT
#define snoise(x) (2 * noise(x) - 1)	
#else
/* prman noise has less range */
#define snoise(x) (2.5 * (noise(x) - 1))
#endif /* BMRT */	
 
surface TLRing (float Ka = 1.0, Kd = 1.0,
...
#define DEBUG 0
	...
	if (val < cutoff * ringrad) {
		...
		relpos =  val / ringrad;
		oi =  (relpos + snoise (40* relpos) - floor(relpos) );
